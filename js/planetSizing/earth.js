$(".earthPictureHolderMob").click(function() {
    console.log("earthclicked");
    
    $(".planetImageHolder").find("img").fadeTo(0, 1);
         if($(".EarthImage").hasClass("enlarged")){
            $(".EarthImage").removeClass("enlarged");
            $(".imagePositionerMob").stop().animate({width: '25vw', height:'25vw'}, 2000 );
            $(".EarthImage").stop().animate({width: '25vw', height:'25vw'}, 2000,
            function(){
            ...
            1;
            var sizeInCm = (sessionStorage.getItem("dpi")/dppx)*0.5393701;
            $(this).parent().addClass('ontop');
            $(".EarthImage").stop().animate({width: sizeInCm, height:sizeInCm}, 2000 );
            $(".imagePositionerMob").stop().animate({height:sizeInCm}, 2000 );
         }
 
 });

 $(".earthPictureHolder").click(function() {
    console.log("earthclicked");
    ...
    $(".planetImageHolder").find("img").fadeTo(0, 1);
         if($(".EarthImage").hasClass("enlarged")){
            ...
            $(".EarthImage").stop().animate({width: '150px', height:'150px'}, 2000,
            function(){
                $(this).parent().removeClass('ontop');
            }); /*so it gets put on bottom AFTER the shrink animation is finished*/
           
            setTimeout(function(){ $(".EarthImage").removeClass("enlarged");},2100);
